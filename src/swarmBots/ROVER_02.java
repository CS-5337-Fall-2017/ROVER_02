...
public class ROVER_02 extends Rover {
	...
	public static void main(String[] args) throws Exception {
		ROVER_02 client;
    	// if a command line argument is present it is used as the IP address for connection to RoverControlProcessor instead of localhost 
	...
	public ROVER_02() {
		// constructor
		System.out.println("ROVER_02 rover object constructed without serverAddress");
		rovername = "ROVER_02";
	}
	
	public ROVER_02(String serverAddress) {
		// constructor
		System.out.println("ROVER_02 rover object constructed with serverAddress");
		rovername = "ROVER_02";
	...
	static enum Direction {
		...
		static HashMap<Character, Direction> map = new HashMap<Character, Direction>() {
			...
			{put('N', NORTH);put('S', SOUTH);put('E', EAST);put('W', WEST);}
		};
	...
	private void run() throws IOException, InterruptedException {
		...
		try {
			...
			System.out.println("Creating socket with SERVER_ADDRESS: " +SERVER_ADDRESS+" PORT_ADDRESS: " + PORT_ADDRESS);
...
*/
				...
				System.out.println("See whats here="+ scanMapTiles[mapTileCenter][mapTileCenter].getScience().toString());
				...
				if (scienceDetail!=null)
				{
					...
					if ( scienceDetail.getX() == getCurrentLocation().xpos	&& scienceDetail.getY() == getCurrentLocation().ypos ) {
						gatherScience( getCurrentLocation() );
						System.out.println( "$$$$$> Gathered science "+ scienceDetail.getScience() + " at location "+ getCurrentLocation() );
					}
					else {
						...
						char dirChar = aStar.findPath( getCurrentLocation(), new Coord( scienceDetail.getX(), scienceDetail.getY() ), driveType );
				...
				moveTargetLocation = chooseMoveTargetLocation(scanMapTiles, currentLocInMapTile, currentLoc, mapTileCenter);
				...
				if (moveTargetLocation != null && moveTargetLocation.d != null) {
					...
					if (!previousLoc.equals(getCurrentLocation())) {
						coordVisitCountMap.put(moveTargetLocation.targetCoord, coordVisitCountMap.get(moveTargetLocation.targetCoord) + 1);
					}
				}
				...
				try {
					...
					System.err.println("Post current map to communication server failed. Cause: "+ e.getClass().getName() + ": " + e.getMessage());
				}
	...
	private boolean isBlocked(MapTile[][] mapTiles, Coord c) {
		return mapTiles[c.xpos][c.ypos].getHasRover() || mapTiles[c.xpos][c.ypos].getTerrain() == Terrain.SAND || mapTiles[c.xpos][c.ypos].getTerrain() == Terrain.NONE;
	}

	private MoveTargetLocation chooseMoveTargetLocation(MapTile[][] scanMapTiles, Coord currentLocInMapTile,Coord currentLoc, int mapTileCenter) {
	...
	private Stack<Direction> getFavoredDirStack(Coord currentLoc, int mapTileCenter) {
		...
		List<Integer> countList = Arrays.asList(northUnvisitedCount, eastUnvisitedCount, southUnvisitedCount,westUnvisitedCount);
		...
		System.out.println("counts = North(" + northUnvisitedCount + ") East(" + eastUnvisitedCount + ") South("+ southUnvisitedCount + ") West(" + westUnvisitedCount + ")");
